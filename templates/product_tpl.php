<div class="flex">
    <div id="left"><?php include _template."layout/left.php"; ?></div>
    <div id="right">
    	<h2 class="title"><?=$title_cat?></h2>
    	<aside class="pd20-0"><?=$title_bar['mota']?></aside>
    	<?=hot_product();?>
	...
	<?php for($i=0,$count_product=count($product);$i<$count_product;$i++){	?>
	    <div class="item item_sp">
            <div class="img_sp hover_sang1 zoom_hinh">
				<a href="<?=$product[$i]['type']?>/<?=$product[$i]['tenkhongdau']?>-<?=$product[$i]['id']?>.html"><img src="thumb/1250x810/1/<?=_upload_sanpham_l.$product[$i]['photo']?>" alt="<?=$product[$i]['ten']?>" /></a>
			</div>
	...
	?>
		...
		<div class="wap_item clearfix">
			<?php for($p=0,$count_product=count($product);$p<$count_product;$p++){	?>
			    <div class="item item_sp">
		            <div class="img_sp hover_sang1 zoom_hinh">
						<a href="<?=$product[$p]['type']?>/<?=$product[$p]['tenkhongdau']?>-<?=$product[$p]['id']?>.html"><img src="thumb/1250x810/1/<?=_upload_sanpham_l.$product[$p]['photo']?>" alt="<?=$product[$p]['ten']?>" /></a>
					</div>
