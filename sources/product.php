<?php  if(!defined('_source')) die("Error");
	...
	else if($id!='')
	{
		...
		//Chi tiết sản phẩm
		$d->reset();
		$sql_detail = "select id,ten$lang as ten,mota$lang as mota,noidung$lang as noidung,masp,gia,giakm,luotxem,thumb,photo,size,mausac,id_danhmuc,id_list,id_cat,title,keywords,description,type FROM #_product where hienthi=1 and id='$id' limit 0,1";
		$d->query($sql_detail);
	...
	{
		$d->reset();
		$sql = "select id,ten$lang as ten,mota$lang as mota,title,keywords,description FROM #_product_item where id='$id_item' limit 0,1";
		$d->query($sql);
	...
	{
		$d->reset();
		$sql = "select id,ten$lang as ten,mota$lang as mota,title,keywords,description FROM #_product_cat where id='$id_cat' limit 0,1";
		$d->query($sql);
		$title_bar = $d->fetch_array();
		if(empty($title_bar)){redirect("http://".$config_url.'/404.php');}
	...
	{
		$d->reset();
		$sql = "select id,ten$lang as ten,mota$lang as mota,title,keywords,description FROM #_product_list where id='$id_list' limit 0,1";
		$d->query($sql);
		$title_bar = $d->fetch_array();
		if(empty($title_bar)){redirect("http://".$config_url.'/404.php');}
	...
	{		
		$d->reset();
		$sql = "select id,ten$lang as ten,mota$lang as mota,title,keywords,description FROM #_product_danhmuc where id='$id_danhmuc' limit 0,1";
		$d->query($sql);
		$title_bar = $d->fetch_array();
		if(empty($title_bar)){redirect("http://".$config_url.'/404.php');}
	...
	$d->reset();
	$sql = "select id,ten$lang as ten,tenkhongdau,thumb,photo,masp,giacu,gia,giakm,type,mota$lang as mota FROM #_product where $where limit $bg,$pageSize";		
	$d->query($sql);
