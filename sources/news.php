<?php  if(!defined('_source')) die("Error");
    ...
    if($id!='')
	{
		...
		//Chi tiết tin tức
		$d->reset();
		$sql_detail = "select id,ten$lang as ten,mota$lang as mota,noidung$lang as noidung,id_danhmuc,id_list,id_cat,title,keywords,description,ngaytao,luotxem,photo FROM #_news where hienthi=1 and id='$id' limit 0,1";
		$d->query($sql_detail);
		$row_detail = $d->fetch_array();

		// lấy dữ liệu từ cấp 1
		// $d->reset();
		// $sql_hinhdm = "select photo FROM #_news_danhmuc where id='".$row_detail['id_danhmuc']."'  and hienthi=1 order by stt,id desc";
		// $d->query($sql_hinhdm);
		// $hinhdm = $d->fetch_array();
	...
	$d->reset();
	$sql = "select id,ten$lang as ten,tenkhongdau,thumb,photo,mota$lang as mota,ngaytao, type FROM #_news where $where limit $bg,$pageSize";		
	$d->query($sql);
